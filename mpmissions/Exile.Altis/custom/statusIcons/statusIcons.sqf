	...
	sb_getIcon = {
		...
		if (_value >=90 && _value < 99 ) then 		{ _icon ="90";		};
		...
		//if (_value >=5 && _value < 10 ) then 		{ _icon ="5";		};
	...
	/*sb_checkTemp = {
			...
			if ( (_bodyTemperature< 30) && ((player getVariable "sb_isCold") isEqualTo false) ) then {
				player setVariable["sb_isCold",true];
				(_display displayCtrl 13375) ctrlSetText "custom\statusIcons\icons\cold.paa";
				[(_display displayCtrl 13375)] call sb_fadeIn;
                [(_display displayCtrl 13395)] call sb_fadeIn;
			};
			
			if ((_bodyTemperature >= 30) && ((player getVariable "sb_isCold") isEqualTo true)) then {
	...
	sb_updateIcons = {
		...
		if (_iconh != "-1") then {
			(_display displayCtrl 13392) ctrlSetText format["custom\statusIcons\circlebar\%1.paa",_iconh];
			[(_display displayCtrl 13392)] call sb_fadeIn;
		};
		
		if (_iconf != "-1") then {
			(_display displayCtrl 13393) ctrlSetText format["custom\statusIcons\circlebar\%1.paa",_iconf];
			[(_display displayCtrl 13393)] call sb_fadeIn;
		};
		
		if (_iconw != "-1") then {
			(_display displayCtrl 13394) ctrlSetText format["custom\statusIcons\circlebar\%1.paa",_iconw];
	...
	sb_init = {
		...
		disableSerialization;
		//systemChat "[ SERVER ] Status Icons werden geladen...";
		player setVariable["sb_info",true];
		player setVariable["infotoggled",false];
		_health = round ((1 - (damage player)) * 100);
		_hunger = round (ExileClientPlayerAttributes select 2);
		...
		_initIcons = {
			...
			_display = (uiNamespace getVariable "StatusIcons");
			_ctrl = _display displayCtrl _idc;
			_ctrl ctrlSetText format["custom\statusIcons\icons\%1.paa",_icon];
		};
